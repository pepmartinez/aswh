...
class HttpClient {
  ...
  _do_http_call (elem, cb) {
    ...
    this._log.verbose ('%s: calling %j', id, wh);
  ...
  _attempt_rearm (elem, res) {
    ...
    try {
      ...
      const delay = _.ceil ((next.getTime () - (new Date ()).getTime ()) / 1000);
      
      this._log.verbose (`rearm: next is ${next.toISOString()}`);

      this._q.push (elem.payload, {delay}, (err, id) => {
        // error while queuing?
        if (err) {
          this._log.error ('error while rearming payload: %o', err);
          this._consumer._metrics.q_ops.labels (this._q.ns(), this._q.name(), 'push', 'ko').inc ();
        }
  
        this._log.info (`rearmed element in queue ${this._q.name()}@${this._q.ns()} with id ${id}, delay is ${delay} secs`);
        this._consumer._metrics.q_ops.labels (this._q.ns(), this._q.name(), 'push', 'ok').inc ();
      });
