  ...
  _do_http_call (elem, cb) {
    ...
    r.end ((err, res) => {
      ...
      if (err) {
        ...
        this._log.info ('%s: (%d mesc) call ok to %s', id, delta, wh.url);
        cb (null, true, res);
      }
    });
  }
      ...
      this._completed_cb_q.push (pl, {}, (err, id) => {
        ...
        this._q.ok (elem, err => {
          if (err) {
            ...
            this._log.verbose ('committed elem %s', elem._id);
  ...
  _manage_process_retry (elem) {
    ...
    this._q.ko (elem, (new Date().getTime () + delay), (err, res) => {
      if (err) {
        ...
        if (res == 'deadletter') {
          this._log.warn ('elem %s moved to deadletter upon retry attempt (too many retries, not really retried)', elem._id);
          this._consumer._metrics.q_ops.labels(this._q.ns(), this._q.name(), 'deadletter', 'ok').inc ();
        }
        else {
          this._log.verbose ('rolled back elem %s', elem._id);
