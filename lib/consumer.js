...
class HttpClient {
  ...
  _do_http_call (elem, cb) {
    ...
    this._log.verbose (`${id}: calling ${wh} with agent ${agent}`);
    ...
    r.end ((err, res) => {
      ...
      if (err) {
        ...
        this._log.info (`${id}: (${delta} msec) got error calling ${wh.url}: ${err.status || err.code || err.errno || err}`);
  ...
  _consume_loop () {
    ...
    this._q.pop('consumer-webhooks', {reserve: true}, (err, elem) => {
      if (err) {
        ...
        this._log.error (`error while popping: ${err}}`);
  ...
  _manage_process_ok (elem, res) {
    ...
    if (elem.payload.cb_url) {
      ...
      this._completed_cb_q.push (pl, {}, (err, id) => {
        ...
        this._q.ok (elem, err => {
          if (err) {
            this._log.error (`error while committing, continuing anyway: ${err}`);
  ...
  _manage_process_retry (elem) {
    ...
    this._q.ko (elem, (new Date().getTime () + delay), (err, res) => {
      if (err) {
        this._log.error (`error while rolling-back, continuing anyway: ${err}`);
  ...
  _manage_process_ko (elem, res) {
    ...
    if (elem.payload.cb_url) {
      ...
      this._failed_q.push (elem.payload, (err, id) => {
        if (err) {
          this._log.error (`can not insert failed element into ${this._failed_q.name()} queue. Error is ${err}, elem is ${elem}`);
